...
const getDataToSignForEIP712 = async (request: any, networkId: number) => {
  ...
  domainData.salt = hexZeroPad(BigNumber.from(networkId).toHexString(), 32);
...
const getDataToSignForPersonalSign = (request: any) => {
  const hashToSign = solidityKeccak256(
    [
      'address',
      'address',
      'address',
 ...
 * @param {*} networkId
 */
const getDomainSeperator = async (networkId: number) => {
  const contractAddresses = await getContractAddresses(networkId);
  const forwarderAddress = contractAddresses.biconomyForwarderAddress;
  let domainData = helperAttributes.biconomyForwarderDomainData;
  domainData.salt = hexZeroPad(BigNumber.from(networkId).toHexString(), 32);
  ...
  const domainSeparator = keccak256(
    defaultAbiCoder.encode(
      ...
      [
        ...
        keccak256(toUtf8Bytes(domainData.name)),
        keccak256(toUtf8Bytes(domainData.version)),
